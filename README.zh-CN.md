...
下面的截图展示了当前的 Vue 操作台界面，以及系统生成的 HTML 研究报告。

| 页面 | 预览 |
| --- | --- |
| **启动页**<br>用于选择运行模式、标的列表、分析日期和数据源。 | ![启动页](startup.png) |
| **历史运行**<br>查看历史运行记录、状态、摘要、产物和快照。 | ![历史运行](history.png) |
| **运行时配置**<br>编辑服务层使用的非敏感运行时配置。 | ![运行时配置](configuration.png) |
| **单标的 HTML 报告**<br>展示单标的工作流、节点轨迹和结构化交易决策。 | ![单标的报告](demo_report_1.png) |
| **多标的 HTML 报告**<br>展示组合图编排、综合分析和目标配置结果。 | ![多标的报告](demo_report_2_multi.png) |
